...
public class TPACommand implements BasicCommand {
    ...
    public TPACommand(PlainBase plugin) {
        this.plugin = plugin;
    }
    ...
    public void execute(@NotNull CommandSourceStack stack, @NotNull String @NotNull [] args) {
        ...
        if (!(args.length == 1)) {
            sender.sendMessage(plugin.getMiniMessage().deserialize(plugin.getMessagesConfig().getString("errors.usage-tpa", "<yellow>Usage: <gray>/tpa <player>")));
            return;
        }
        ...
        if (target.equals(player)) {
            sender.sendMessage(plugin.getMiniMessage().deserialize(plugin.getMessagesConfig().getString("errors.teleport-self", "<red>You cannot teleport to yourself!")));
            return;
        }
